...
class DecoderRNN(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        # training a lstm model - sequence info is just used for cell and hidden
        # state. Input provided, during time step i + 1, is the ith word in the
        # sequence.
        ...
        # Embedding the words in the caption -> batch_size, caption_len - 1,
        # embed_size.
        captions_embed = self.word_embeddings(captions)

        # Appending features as the first input.
        inputs = torch.cat((features.view(batch_size, 1, -1), captions_embed),
                           dim = 1)
    ...
    def sample(self, features, states=None, max_len=20, end_token=1):
        ...
        while length <= max_len:
            ...
            # Converting the word integer back to input (embed) for next
            # timestep
